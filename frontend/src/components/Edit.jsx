...
const Edit = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
        try {
            ...
            const [countryRes, leagueRes, charRes, clubRes] = await Promise.all([
                AxiosInstance.get(`countries/`),
                AxiosInstance.get(`leagues/`),
                AxiosInstance.get(`characteristics/`),
                AxiosInstance.get(`footballclubs/${MyId}/`)
  ...
  const formik = useFormik({
    ...
    onSubmit: (values) => {
      AxiosInstance.put(`footballclubs/${MyId}/`, values).then(() => {
        setMessage(
          <MyMessage
            messageText={"You succesfully updated the football club!"}
            messagecolor={"green"}
          />
        );
        setTimeout(() => {
